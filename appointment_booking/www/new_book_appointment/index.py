...
def get_services(name):
	# results = frappe.db.sql(""" select duration,price,date from `tabService Duration and Price` where parent=%s order by price asc""",(name),as_dict=True)
	results = frappe.db.sql(""" select c.duration,c.price,p.name,p.service_provider from `tabAvailable Services List` as p left join `tabPrice Of Services` as c on p.name=c.parent where c.parent=%s order by c.price asc""",(name),as_dict=True)
	return results
...
@frappe.whitelist(allow_guest=True)
def create_appointment(date, time, tz, contact,service_provider, appointment_title, duration, pika):
	format_string = "%Y-%m-%d %H:%M:%S"
	...
	appointment.service_provider = service_provider
	appointment.duration = duration
	appointment.price = pika
